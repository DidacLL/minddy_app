...
export function MTextInput(props: MTextInputParams) {
    ...
    return <div className={`z-10 relative overflow-visible w-auto form-control input-group-sm h-min rounded-box py-2 pl-1 pr-3 m-2 ${showError && 'input-error'} ${props.clazzName}`}>
        ...
        <input type="text" placeholder={props.placeHolder} value={currentValue} onChange={handleChange}
               className={`ml-10 mb-0 bg-base-300  input-sm input input-bordered max-w-prose w-max resize`}/>
        ...
        <div className='absolute -z-10 mask-frame bg-base-300 opacity-50  w-full h-full '></div>
    </div>;
}
